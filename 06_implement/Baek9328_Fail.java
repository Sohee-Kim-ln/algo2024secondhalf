...
public class Baek9328_Fail {
	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			for (int i = 1; i <= K; i++) {
				// 다음칸 색에 따른 이동 실시
				System.out.println(cnt + "턴 " + i + "번 말 움직임");
				move(i);
	...
	static void move(int idx) {
		...
		System.out.println(idx + "번말 상태" + r + "," + c + " 방향" + chessman[idx][2]);

		// 다음칸 계산
		int nextr = r + dr[chessman[idx][2]];
		int nextc = c + dc[chessman[idx][2]];
		System.out.println("다음칸 " + nextr + "," + nextc);

		color = board[nextr][nextc].color;

		// 파란 색일때 방향 반전 후 다음칸 재계산
		if (color == 2) {
			System.out.println("다음칸 파랑이라 뒤집기");
		...
		System.out.println("현재노드 갱신전" + board[r][c].toString());
		System.out.println("다음노드 갱신전" + board[nextr][nextc].toString());
		...
		if (color == 2) {
			System.out.println("뒤집어도 파랑이라 종료");
			return;
		}
		...
		if (color == 1) {
			// 위에서부터 거꾸로 읽어서 저장
			for (int i = cur.last; i >= chessman[idx][3]; i--) {
		...
		System.out.println("현재노드" + cur.toString());
		System.out.println("다음노드" + next.toString());
		System.out.println();

		// 말이 4개이상 쌓였으면 종료
		if (next.last >= 3) {
			System.out.println("4개이상 쌓임");
			isOver = true;
			return;
		}
	}
